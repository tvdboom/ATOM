...
<a href="https://github.com/tvdboom/ATOM/blob/master/atom/basepredictor.py#L197">[source]</a>
</span>
</div>

Transform new data through all transformers in the current branch and
return predicted confidence scores. If called from a trainer, the best
model in the pipeline (under the `winner` attribute) is used. If called
from a model, that model is used. The estimator must have a
