...
Read more in sklearn's [documentation](https://scikit-learn.org/stable/modules/generated/sklearn.naive_bayes.BernoulliNB.html).
...
Predicted class log-probabilities on the training set.
</p>
<p>
<strong>predict_log_proba_test: pd.DataFrame</strong><br>
Predicted class log-probabilities on the test set.
</p>
<p>
...
or `atom.bnb.predict(X)`. The remaining utility methods can be found hereunder.
<br><br>
...
atom.run(models="BNB", metric="precision")
```