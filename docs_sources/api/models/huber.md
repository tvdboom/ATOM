...
Read more in sklearn's [documentation](https://scikit-learn.org/stable/modules/linear_model.html#huber-regression).
...
<p>
<strong>epsilon: float, default=1.35</strong><br>
...
can be called directly from the model, e.g. `atom.hub.plot_residuals()`
or `atom.hub.predict(X)`. The remaining utility methods can be found
hereunder.
