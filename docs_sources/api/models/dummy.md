...
Read more in sklearn's [documentation](https://scikit-learn.org/stable/modules/model_evaluation.html#dummy-estimators).
...
can be called directly from the models, e.g. `atom.dummy.plot_roc()` or
`atom.dummy.predict(X)`. The remaining utility methods can be found hereunder.
...
atom.run(models=["Dummy", "Tree"])  # Compare baseline with a decision tree
```