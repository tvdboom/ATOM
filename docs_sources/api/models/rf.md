...
or mean prediction (regression) of the individual trees. Random forests
...
Categorical(["squared_error", "absolute_error", "poisson"], name="criterion")</li>
</ul>
<p>
...
<p>
<strong>bootstrap: bool, default=False</strong><br>
...
can be called directly from the models, e.g. `atom.rf.plot_permutation_importance()` or `atom.rf.predict(X)`.
...
atom.run(models="RF", metric="mae", n_calls=20, n_initial_points=10)
```