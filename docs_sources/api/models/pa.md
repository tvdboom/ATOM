...
Read more in sklearn's [documentation](https://scikit-learn.org/stable/modules/linear_model.html#passive-aggressive).
...
<p>
<strong>average: float, default=False</strong>
...
can be called directly from the models, e.g. `atom.pa.plot_permutation_importance()` or `atom.pa.predict(X)`.
