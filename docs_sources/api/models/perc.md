...
to train than [SGD](../sgd) with the hinge loss and that the resulting
models are sparser.
...
Read more in sklearn's [documentation](https://scikit-learn.org/stable/modules/linear_model.html#perceptron).
...
* The `n_jobs` and `random_state` parameters are set equal to those of the parent.
...
<p>
<strong>penalty: str or None, default=None</strong><br>
...
<p>
<strong>max_iter: int, default=1000</strong><br>
Integer(500, 1500, name="max_iter")
</p>
<p>
<strong>eta0: float, default=1.0</strong><br>
...
can be called directly from the model, e.g. `atom.perc.plot_roc()`
or `atom.perc.predict(X)`. The remaining utility methods can be
found hereunder.
