...
class ATOM(object):
    ...
    def impute(self,
        ...
        strat_num: str, int or float, optional (default='remove')
            Imputing strategy for numerical columns. Choose from:
                ...
                - Any number to fill the missing values with

        strat_cat: str, optional (default='remove')
            Imputing strategy for categorical columns. Choose from:
                - 'remove': remove row if any missing value
                - 'most_frequent': fill with most frequent value
                - Any string to fill the missing values with
        ...
        missing: int, float or list, optional (default=None)
            List of values to impute. None for default list.
    ...
    def encode(self, max_onehot: Optional[int] = 10, frac_to_other: float = 0):
        ...
        target-encoding for n_unique > max_onehot. It also can replace classes
    ...
    def outliers(self,
        ...
        max_sigma: int or float, optional (default=3)
            Remove rows containing any value with a maximum standard deviation
            (on the respective column) above max_sigma.
    ...
    def pipeline(self,
        ...
        needs_proba: bool, optional (default=False)
            Whether the metric function requires predict_proba to get
            probability estimates out of a classifier. Will be ignored if the
        ...
        def run_iteration():
            ...
            try:  # Fails if all models encountered errors
                ...
                for m in self.models:
                    if bagging is None:
                        scores.append(getattr(self, m).score_test)
                    else:
                        scores.append(getattr(self, m).bagging_scores.mean())
            ...
            for m in self.models:
                ...
                if bagging is None:
                    ...
                    # Highlight best score and assign winner attribute
                    if bs_mean == max(scores) and len(self.models) > 1:
                        self.winner = getattr(self, m)
                        print_ += ' !!'
