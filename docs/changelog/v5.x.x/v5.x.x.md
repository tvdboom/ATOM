...
* The `drop` method is removed from atom. Use the reworked [apply][atomclassifier-apply]
  method instead.
* The [prediction methods][predicting] can no longer be called from atom.
...
* The [Normalizer][] class can now be accelerated with [GPU][estimator-acceleration].
...
* The [prediction methods][predicting] now return pandas objects.
...
* Fixed a bug where models could not call the [score][adaboost-score] function.
