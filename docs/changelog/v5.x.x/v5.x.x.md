...
* New [backend][atomclassifier-backend] parameter to choose a [parallel execution][]
  backend.
* It's now possible to train models in parallel.
* New [serve][adaboost-serve] method to deploy models to a rest API endpoint.
* New [get_best_threshold][adaboost-get_best_threshold] method calculate the optimal
  threshold for binary and multilabel tasks.
